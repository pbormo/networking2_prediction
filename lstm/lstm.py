...
def train_and_evaluate_per_group(data_folder, output_folder, sequence_length=5, epochs=50, batch_size=16):
    ...
    #for (switch, source_port, dest_port, protocol), group in df.groupby(['Switch ID', 'Source IP', 'Destination IP', 'Protocol']):
        ...
        for i in range(len(group_scaled) - sequence_length):
            ...
            labels.append(group_scaled[i + sequence_length][0])  # take only throughput
